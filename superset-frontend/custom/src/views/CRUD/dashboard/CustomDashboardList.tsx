...
import CheckboxApiComponent from '../../../components/DefaultDashboardCheckBox';
const PAGE_SIZE = 25;
...
function DashboardList(props: DashboardListProps) {
  ...
  // custom code added starting from here
  // new set default dashboard states
  const [selectedDashboardId, setSelectedDashboardId] = useState<number | null>(null);
  // handle api post request
  async function saveCheckboxApi(dashboardId: number) {
    try {
      setSelectedDashboardId(dashboardId);
      return SupersetClient.post({
        endpoint: `/api/v1/customrestapi/welcome_dashboard/${dashboardId}`,
      })
        .then(({ json = {} }) => {
          addSuccessToast(json.message);
        })
  ...
  async function fetchDefaultDashboardId() {
    try {
      const { json = {} } = await SupersetClient.get({
        endpoint: '/api/v1/customrestapi/welcome_dashboard/',
      });
      const dashboardId = json.result.dashboard_id;
      setSelectedDashboardId(dashboardId);
      return dashboardId;
    } catch (error) {
      const errorMessage = error?.response?.data?.message || '';
  ...
  useEffect(() => {
    ...
    async function fetchData() {
      const dashboardId = await fetchDefaultDashboardId();
      if (!ignore) {
        setSelectedDashboardId(dashboardId);
      }
    }
  ...
  const handleCheckboxClick = useCallback(
    async (dashboardId: number) => {
      ...
      await saveCheckboxApi(dashboardId);
    },
    [selectedDashboardId, saveCheckboxApi]
  );
  ...
  const columns = useMemo(
    () => {
            ...
            userId && (
              <React.Fragment>
                <CheckboxApiComponent
                  id={original.id}
                  isChecked={selectedDashboardId === original.id}
                  saveCheckboxApi={handleCheckboxClick}
                  fetchCheckboxApi={fetchDefaultDashboardId}
                  showTooltip={true}
                  defaultChecked={setSelectedDashboardId === original.id}
                />
              </React.Fragment>
            ),
          size: 'xs',
      ...
      const finalColumns = canCreate ? [...columnsArray, ...conditioncolumnsArray] : [...columnsArray];
      return finalColumns;
    },
    [
      userId,
